...
!!! note
    ...
    Убедитесь что на сцене только одна активная камера, так как их избыток увеличивает нагрузку на GPU. Для этого, выполните поиск по компоненту `t: Camera` в окне Hierarchy.
