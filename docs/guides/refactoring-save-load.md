...
Активируйте паузу в играх в реальном времени, где потеря контроля ведет к гибели персонажа (шутеры, файтинги), потере ресурсов (RTS), пропуску таймеров в UI или ошибкам в таблицах лидеров (гонки, симуляторы).
