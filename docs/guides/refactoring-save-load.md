...
Активируйте паузу в играх в реальном времени, где потеря контроля ведет к смерти персонажа (шутеры, файтинги), потере ресурсов (RTS), пропуску таймеров в UI или ошибкам в таблицах лидеров (гонки, симуляторы).
